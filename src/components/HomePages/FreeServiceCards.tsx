  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth < 640) {
        setCardWidth(300); // Smaller width for mobile
      } else if (window.innerWidth < 1024) {
        setCardWidth(350); // Medium width for tablets
      } else {
        setCardWidth(420); // Default width for desktop
      }
    };
    ...
    <div className="w-full overflow-hidden py-12 bg-gray-100 relative">
      ...
      >
        {cards.map((card) => (
          <div
            key={card.id}
            className="relative w-[300px] sm:w-[350px] lg:w-[400px] h-[200px] sm:h-[240px] lg:h-[260px] flex-shrink-0 rounded-lg overflow-hidden"
          >
            {/* Card Image */}
            <img
              src={card.image}
